...
# linus 非常奇怪，添加下面一行，代码已经可以正常run了，但为何reducible_colors、functions下面还有波浪线
sys.path.append("../common/")
# linus 为了不显示波浪线，选择自动fix，添加了一份.vscode文件（单独添加这份文件代码无法run）
# linus 原来vscode还能自动fix bug
...
#linus 本来一开始common文件夹下的3份文件都无法导入，结果自动fix，添加了一个.vscode文件就解决了

#linus 本来下载的是reducible的完整代码库，但是会有些变量无法识别，比如Scene，就很莫名奇妙。
#linus 怀疑是和reducible的其他文件夹下的名字冲突。所以，将代码拷贝到单独的文件夹。果然，名字冲突消失了。

# linus 思考一下本质是什么？
# python的函数定义不指定参数的类型，也不指定返回值的类型。但在其他静态语言，这都是必须指定的。
# python尽管不强制，但是可以软性告知下。相当于是一个妥协。
"""linus
函数参数中的冒号是参数的类型建议符，告诉程序员希望传入的实参的类型。
函数后面跟着的箭头是函数返回值的类型建议符，用来说明该函数返回的值是什么类型。
值得注意的是，类型建议符并非强制规定和检查，也就是说即使传入的实际参数与建议参数不符，也不会报错，但会有警告。
...
class IntroWebGraph(Scene):
    ...
    def get_web_graph(self):
        ...
        #linus MarkovChain是为了初始化MarkovChainGraph
...
#linus manim -pql markov_chain.py IntroImportanceProblem
